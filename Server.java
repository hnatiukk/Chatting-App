    ...
    private static class Handler extends Thread {
        ...
        public void run() {
            ...
            try (Connection connection= new Connection(socket)){
                current = serverHandshake(connection);
                sendBroadcastMessage(new Message(MessageType.USER_ADDED, current));
                notifyUsers(connection, current);
                serverMainLoop(connection, current);

            } catch (IOException | ClassNotFoundException e) {
                ConsoleHelper.writeMessage("Ошибка при обмене данными с " + socket.getRemoteSocketAddress());
            }
            if (current != null) {
                connectionMap.remove(current);
                sendBroadcastMessage(new Message(MessageType.USER_REMOVED, current));
            }
